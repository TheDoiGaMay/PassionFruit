...
function BreakBlock(Position)
	...
	if block and block.Parent ~= nil then
		if (LocalPlayer.Character.HumanoidRootPart.Position - (blockpos * 3)).magnitude > 30 then
			return
		end
		...
		task.spawn(function()
			local animation
			animation = BedwarLibrary.AnimationUtil:playAnimation(
				LocalPlayer,
				BedwarLibrary.BlockController:getAnimationController():getAssetId(1)
			)
		...
		task.spawn(function()
			BedwarLibrary.ClientHandlerDamageBlock
				:Get("DamageBlock")
				:CallServerAsync({
				...
				:andThen(function(result)
					if result ~= "failed" then
						...
						then
							local blockdata =
								BedwarLibrary.BlockController:getStore():getBlockData(blockhealthbarpos.blockPosition)
							local blockhealth = blockdata and blockdata:GetAttribute(LocalPlayer.Name .. "_Health")
						...
						BedwarLibrary.BlockBreaker:updateHealthbar(
							blockhealthbarpos,
						...
						if healthbarblocktable.blockHealth <= 0 then
							BedwarLibrary.BlockBreaker.breakEffect:playBreak(
								block.Name,
								blockhealthbarpos.blockPosition,
								LocalPlayer
							)
							BedwarLibrary.BlockBreaker.healthbarMaid:DoCleaning()
							healthbarblocktable.breakingBlockPosition = Vector3.zero
						else
							BedwarLibrary.BlockBreaker.breakEffect:playHit(
								block.Name,
								blockhealthbarpos.blockPosition,
								LocalPlayer
							)
...
BedwarLibrary = {
	...
	DefaultKillEffect = require(
		LocalPlayer.PlayerScripts.TS.controllers.game.locker["kill-effect"].effects["default-kill-effect"]
	),
	...
	BlockEngineClientEvents = require(
		ReplicatedStorage["rbxts_include"]["node_modules"]["@easy-games"]["block-engine"].out.client["block-engine-client-events"]
		...
		function(args)
			if args == true then
				AutoclickerConnection = game:GetService("RunService").Heartbeat:Connect(function()
					...
					if GetCurrentState.Game.matchState == 0 then
						return
					end
					if pressed then
					else
					...
					if IsHoveringOnGui() then
						return
					end
					if workspace:GetServerTimeNow() < NextClickTimer then
						return
					end
					if BedwarLibrary.KatanaController.chargingMaid == nil then
					else
						return
					end
					...
					elseif
						getEquipped()["Type"] == "block"
						and GetDoAllowPlaceBlock
						and DoNotPlaceAnyBlock == false
					then
						local mouseinfo = BedwarLibrary.BlockPlacementController.blockPlacer.clientManager
							:getBlockSelector()
							:getMouseInfo(0)
						if mouseinfo then
							if not StartPlacing then
								...
								if (tick() - StartPlacing) > (1 / 12) then
									if BedwarLibrary.BlockPlacementController.blockPlacer then
										BedwarLibrary.BlockPlacementController.blockPlacer:placeBlock(
...
do
	...
	local function PlayerAdded(PlayerAddedthingy)
		Connectionlist[PlayerAddedthingy] = RunService.Heartbeat:Connect(function(args)
			if
				isAlive(PlayerAddedthingy)
				and not PlayerAddedthingy.Character:FindFirstChild("wkaodmkads")
				and not (PlayerAddedthingy.TeamColor == LocalPlayer.TeamColor)
			then
	...
	Combattab:newmod(
		{
			ModName = "Hitboxes",
			ModDescription = "Lazy of aiming at character? aim at the hitbox instead!",
			Keybind = "None",
		},
		function(args)
			if args == true then
...
do
	...
	Combattab:newmod(
		{
			ModName = "Velocity",
			ModDescription = "Hate current Knockback system when u just bought 1K USD PC for kb advantage?? Passionfruit make it better and cheaper!",
			Keybind = "None",
		},
		function(args)
			if args == true then
				...
				BedwarLibrary.KnockbackUtil.applyKnockback = function(root, mass, dir, knockback, ...)
					...
					if GetHorValue == 0 and GetVertValue == 0 then
						return
					end
...
do
	...
	UtilityTab:newmod(
		{
			ModName = "Auto Tool",
			ModDescription = "Best module of switching breaking tool when you need! (Note: You need to switch to any tool first)",
			Keybind = "None",
		},
		function(args)
			if args == true then
					...
					if isAlive() then
						for i, v in next, Players:GetPlayers() do
							if
								isAlive(v)
								and v ~= LocalPlayer
								and v.Character:FindFirstChild("HannahExecuteInteraction")
								and v.Character:FindFirstChild("HannahExecuteInteraction").Enabled == true
								and (
										LocalPlayer.Character.HumanoidRootPart.Position
										- v.Character.PrimaryPart.Position
									).Magnitude
									<= v.Character:FindFirstChild("HannahExecuteInteraction").MaxActivationDistance
							then
								if CanExecute then
									CanExecute = false
									...
									task.spawn(function(args)
										task.wait(
											v.Character:FindFirstChild("HannahExecuteInteraction").HoldDuration + 0.1
										)
										CanExecute = true
									end)
...
do
	...
	ReplicatedStorage.rbxts_include.node_modules["@rbxts"].net.out._NetManaged.EnchantTableResearch.OnClientEvent:Connect(
		function(args)
	...
	UtilityTab:newmod(
		{
			ModName = "Kaliyah Auto Punch",
			ModDescription = "You will punch the enemy whenever their health is below damage calculation",
			Keybind = "None",
		},
		function(args)
			if args == true then
				KaliyahConnection = RunService.Heartbeat:Connect(function()
					...
					for i, v in next, Players:GetPlayers() do
						if
							isAlive(v)
							and v ~= LocalPlayer
							and v.Character:FindFirstChild("KaliyahPunchInteraction")
							and v.Character:FindFirstChild("KaliyahPunchInteraction").Enabled == true
							and (LocalPlayer.Character.HumanoidRootPart.Position - v.Character.PrimaryPart.Position).Magnitude
								<= v.Character:FindFirstChild("KaliyahPunchInteraction").MaxActivationDistance
						then
							IsVisiblable = true
							raycastParams.FilterDescendantsInstances = { workspace:WaitForChild("Map") }

							local VictimLocation = v.Character.PrimaryPart.Position - Vector3.new(0, 2.5, 0)
							local CharacterLocation = Vector3.new(
								LocalPlayer.Character.PrimaryPart.Position.X,
								VictimLocation.Y,
								LocalPlayer.Character.PrimaryPart.Position.Z
							)

							local DisplayPosition = (CFrame.new(
								VictimLocation,
								(CFrame.new(CharacterLocation, VictimLocation) * CFrame.new(0, 0, -299)).Position
							) * CFrame.new(0, 0, -(3 * 7))).Position
							local VictimLocation2 = (CFrame.new(
								VictimLocation,
								(CFrame.new(CharacterLocation, VictimLocation) * CFrame.new(0, 0, -299)).Position
							) * CFrame.new(0, 0, -(3 * 6))).Position

							local RaycastLocation1 = CFrame.new(CharacterLocation, VictimLocation).LookVector.Unit
								* (3 * 7)
							...
							local newraycastresult2 =
								workspace:Raycast(VictimLocation2, RaycastLocation2, raycastParams)

							att0.Position = VictimLocation

							local GetTheDamageUpgrade = shared.BedwarTable["ClientStoreHandler"]:getState().Bedwars.teamUpgrades["damage"]
								or -1
							...
							local IsAnywayToggled =
								shared.IClientToggledProperty["Kaliyah Auto Punch"]["Punch anyway when detect wall or void"]

							local PunchDamage = raycastResult and (IsAnywayToggled and math.huge or 28)
								or not raycastResult and not newraycastresult2 and math.huge
								or 6
							...
							if v.Character.Humanoid.Health <= (PunchDamage + ExtraDamage) + FireDamage then
								if CanBeExecute then
									CanBeExecute = false
									task.delay(
										v.Character:FindFirstChild("KaliyahPunchInteraction").HoldDuration,
										function(args)
											CanBeExecute = true
										end
									)
...
do
	...
	function RenderItem(ItemPrimaryPart, icon)
		...
		image.Image = BedwarLibrary.getIcon(
			{ itemType = (icon == "crit_star" and StarsIconIndex[ItemPrimaryPart.Parent.Name] or icon) },
			true
		)
	...
	UtilityTab:newmod(
		{
			ModName = "Kit ESP",
			ModDescription = "Show where all the item are! so u dont have to walk around the map 24/7",
			Keybind = "None",
		},
		function(args)
			if args == true then
				...
				if IconIndex[GetCurrentBedwarsEquippedKid] and KitItemTag[GetCurrentBedwarsEquippedKid] then
					ESPConnection.InstanceAdded = CollectionService
						:GetInstanceAddedSignal(KitItemTag[GetCurrentBedwarsEquippedKid])
						:Connect(function(newObject)
							RenderItem(newObject.PrimaryPart, IconIndex[GetCurrentBedwarsEquippedKid])
						end)

					ESPConnection.InstanceRemoved = CollectionService
						:GetInstanceRemovedSignal(KitItemTag[GetCurrentBedwarsEquippedKid])
						:Connect(function(v)
		...
		function(args)
			if args == true then
				NoParticleConnection = workspace.Explosions.ChildAdded:Connect(function(object)
					if not shared.IClientToggledProperty["Explosion"]["No Explosion Particle"] then
						return
					end
					task.wait()
				...
				ShowTNTRadiusConnection = CollectionService:GetInstanceAddedSignal("tnt"):Connect(function(obj)
					if not shared.IClientToggledProperty["Explosion"]["Show Explosion Radius (Not Accurate)"] then
						return
					end
					...
					killaurarangecirclepart.Transparency = shared.IClientToggledProperty["Explosion"]["Radius Transparency"]
						/ 100
...
do
	...
	UtilityTab:newmod(
		{
			ModName = "GingerbreadMan",
			ModDescription = "Tired of quick hotkeying + tryharding with ginger? just use this ez peezee",
			Keybind = "None",
			BindOnly = true,
		},
		function(args)
			...
			local pos = Vector3.new(
				...
				RoudUpPosition(
					Vector3.new(
						0,
						CurrentPlayerPosition.Y - (((CurrentPlayerHrootSize.Y / 2) + CurrentHumanoid.HipHeight) - 1.5),
						0
					)
				).Y,
...
do
	...
	local function GetrandomPosition()
		...
		local CalculatedPosition = Vector3.new(
			RandomCFrame.Position.X,
			RoudUpPosition(
				Vector3.new(
					0,
					RandomCFrame.Position.Y - (((CurrentPlayerHrootSize.Y / 2) + CurrentHumanoid.HipHeight) - 1.5),
					0
				)
			).Y,
			RandomCFrame.Position.Z
		)
				...
				for i, v in pairs(FlagList) do
					if getItem(v) then
						...
						if not PlacedFlag[v] then
							...
							task.spawn(function()
								PlaceBlockEngine:placeBlock(
									Vector3.new(
										GetTheRandomPosition.X / 3,
										GetTheRandomPosition.Y / 3,
										GetTheRandomPosition.Z / 3
									)
								)
							end)
							task.wait((1 / 10))
		...
		function(args)
			if args then
				...
				BedwarLibrary.FovController.setFOV = function(self, fov)
					print("FOV CHANGED")
					return OldSetFov(
						BedwarLibrary.FovController,
						BedwarLibrary.ClientStoreHandler:getState().Settings.fov
					)
				end
...
do
	...
	CosmeticTab:newmod(
		{
			ModName = "Better Combat Sound",
			ModDescription = "You will feel so satisfied when fighting while turning this on",
			Keybind = "None",
		},
		function(args) end,
		{}
...
do
	...
	CosmeticTab:newmod(
		{
			ModName = "Better Combat Sound Part 2",
			ModDescription = "You will feel so satisfied when fighting while turning this on",
			Keybind = "None",
		},
		function(args)
...
do
	...
	table.sort(KillEffectName, function(a, b)
		return a:lower() < b:lower()
	end)

	CosmeticTab:newmod(
		{
			ModName = "Kill Effect Adjuster",
			ModDescription = "Change to any kill effect you want beside dont having it",
			Keybind = "None",
		},
		function(args) end,
		{
...
do
	local BedBreakEffectFunction = {
		["Bed Firework (Grilwar)"] = function(Position)
			...
			TweenService:Create(
				...
				{ Position = CloneBed.Position + Vector3.new(0, 45, 0) }
			):Play()
			...
			KnitClient.Controllers.FireworkController:playFireworkEffect(
				CloneBed.Position
					+ Vector3.new(math.random() * 7 - 3.5, math.random() * 7 - 3.5, math.random() * 7 - 3.5),
				4,
				{
					sizeMultiplier = 1.5,
			...
			local v8 = ReplicatedStorage:WaitForChild("Assets")
				:WaitForChild("Effects")
				:WaitForChild("ConfettiParticle")
				:Clone()
			v8.Enabled = false
			v8.Parent = firework
		...
		["Portal Yoinking Bed"] = function(Position)
			...
			TweenService:Create(mesh, TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), tweentable)
				:Play()
			TweenService:Create(ring, TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), tweentable2)
				:Play()
			...
			TweenService:Create(mesh, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), tweentable)
				:Play()
			TweenService
				:Create(part, TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), tweentable2)
				:Play()
			task.wait(0.35)
			Connection:Disconnect()
			CloneBed:Destroy()
	...
	CosmeticTab:newmod(
		{
			ModName = "Bed Destroy Effect",
			ModDescription = "bed broken effect like Hypixel vibe? hell yeah!",
			Keybind = "None",
		},
		function(args)
			if args == true then
				Connection = game:GetService("ReplicatedStorage").rbxts_include.node_modules["@rbxts"].net.out._NetManaged.BedwarsBedBreak.OnClientEvent
					:Connect(function(FireData)
						local GetSelectedBedEffect =
							shared.IClientToggledProperty["Bed Destroy Effect"]["Selected bed broken effect"]
...
do
	...
	BedwarLibrary["ClientHandler"]:OnEvent("EntityDamageEvent", function(p3)
		...
		if p3.entityInstance:FindFirstChild("_DamageHighlight_") then
		else
			return
		end
		if p3.fromEntity == LocalPlayer.Character then
			if shared.IClientToggledProperty["IWannaSleep"]["Random Color"] == true then
				p3.entityInstance:FindFirstChild("_DamageHighlight_").FillColor =
					Color3.fromRGB(math.random(1, 255), math.random(1, 255), math.random(1, 255))
			else
				local R, G, B =
					shared.IClientToggledProperty["IWannaSleep"]["Red"],
					shared.IClientToggledProperty["IWannaSleep"]["Green"],
					shared.IClientToggledProperty["IWannaSleep"]["Blue"]
...
do
	...
	table.sort(SetEmoteName, function(a, b)
		return a:lower() < b:lower()
	end)
			...
			if args == true then
				if not (shared.IClientToggledProperty["Emote Adjuster"]["Selected Emote"] == "") then
					task.spawn(function()
						...
						BedwarLibrary.ClientStoreHandler:getState().Locker.selectedSpray =
							SetEmoteName2[shared.IClientToggledProperty["Emote Adjuster"]["Selected Emote"]]
...
do
	...
	BlantantTab:newmod({ ModName = "Fake Lag", ModDescription = "Uhm idk", Keybind = "None" }, function(args)
		if args == true then
			TheConnection = RunService.Heartbeat:Connect(function()
				local IsStillFakeLag =
					shared.IClientToggledProperty["Fake Lag"]["still do fake lag but not that much when not near player"]
						...
						if v == LocalPlayer then
							...
							if v.TeamColor == LocalPlayer.TeamColor then
								...
								if
									...
									and (
											v.Character.PrimaryPart.Position - LocalPlayer.Character.PrimaryPart.Position
										).Magnitude
										< Radius
								then
									IsNear = true
				...
				if LagToWhatTime < tick() and tick() > TimeToStartFakeLag then
					LagToWhatTime = tick()
						+ (shared.IClientToggledProperty["Fake Lag"]["Spoof Time"] / 100)
							* (NearPlayerOnly and IsNear == false and IsStillFakeLag and 0.5 or 1)
					TimeToStartFakeLag = tick()
						+ (
							(shared.IClientToggledProperty["Fake Lag"]["Spoof Each Delay"] / 100)
							+ (shared.IClientToggledProperty["Fake Lag"]["Spoof Time"] / 100)
						)
				end
				...
				if LagToWhatTime > tick() then
					game:GetService("NetworkClient")
						:SetOutgoingKBPSLimit((NearPlayerOnly and IsNear == false and IsStillFakeLag and 4 or 1))
